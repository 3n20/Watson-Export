  ...
  var dt = new Date();
   ...
   //};
    ...
    conversationLog(params).then(function(linhas){
        fs.writeFile("./views/extract/export"+ dtn +".csv", linhas, 'ascii', function(err) {
            if(err) {
                console.log(err);
            } else {
                var pathAbsolute = path.resolve('./views/extract/export.csv');
                res.send('http://10.40.0.99:3001/extract/export.csv');
            }
        });
    }).catch(function(e){
        console.log(e);
    ...
    return new Promise(function(accept, reject){
        assistant.listLogs(params, function(err, response) {
            if (err) {
                ...
                linha = "Mensagem" + ";" + "Data" + ";" + "Intenção" + ";" + "Confianca"+ ";" + "Entidades"+ ";" + "Resposta" + "\r";
                for(var i=0; json.length > i; i++){
                    ...
                    linha = linha + log + ";" + time  + ";" + intencao + ";" + confianca+ ";" + entidades+ ";" + resp + "\r"
